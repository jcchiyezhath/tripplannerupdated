const CSV_URL =
  "https://docs.google.com/spreadsheets/d/e/2PACX-1vQlvP8j5dZDbEghxDiom6ByE62ccsp7NNCAa4HrPw58dp4_8A3WKZHqOFFIVMDNoeITTh8CPJbTUvDH/pub?gid=1738934819&single=true&output=csv";
...
fetch(CSV_URL)
  ...
  .then(csv => {
    ...
    renderCards(headers, items, "cost-list");
  })
  .catch(err => {
    console.error("Cost CSV error:", err);
    const p = document.getElementById("cost-error");
    if (p) {
      p.style.display = "block";
      p.textContent = "Could not load cost. Check the CSV link + publish settings.";
    }
  });
